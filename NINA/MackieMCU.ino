...
*/
  ...
  void MackieMCU_handlePitchBendChange(uint8_t midiChannel, int value) {
    if ((midiChannel >= 1) && (midiChannel <= 8)) {
      ...
      if (MackieMCU.channel[channel].faderTouched) {
        ...
        audiomixer.chVolume[channel] = newVolume;
        executeCommand("mixer:volume:ch" + String(channel + 1) + "@" + String(newVolume, 2));
      }
      ...
      if (MackieMCU.channel[32].faderTouched) {
        ...
        audiomixer.mainVolume = newVolume;
        executeCommand("mixer:volume:main@" + String(newVolume, 2));
      }
    }
  }
  ...
  void MackieMCU_sendData() {
    ...
    String playtime = secondsToHMS_B(audio.getAudioCurrentTime(), false); // 00:00:00
    String valueString;
    ...
    for (uint8_t i_ch=MackieMCU.channelOffset; i_ch<(8+MackieMCU.channelOffset); i_ch++) {
      newFaderValue = ((audiomixer.chVolume[i_ch] + 48.0f)/54.0f) * 16383.0f;
    ...
    // MasterFader
    newFaderValue = ((audiomixer.mainVolume + 48.0f)/54.0f) * 16383.0f;
